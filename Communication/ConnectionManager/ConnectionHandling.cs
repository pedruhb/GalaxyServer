...
namespace Galaxy.Communication.ConnectionManager
{
    public class ConnectionHandling
	{
		...
		private void manager_connectionEvent(ConnectionInformation connection)
		{
			...
			GalaxyServer.GetGame().GetClientManager().CreateAndStartClient(Convert.ToInt32(connection.getConnectionID()), connection);
		}
		...
		private void CloseConnection(ConnectionInformation Connection)
		{
			...
			{
				Connection.Dispose();
				GalaxyServer.GetGame().GetClientManager().DisposeConnection(Convert.ToInt32(Connection.getConnectionID()));
			}
			catch (Exception e)
			{
