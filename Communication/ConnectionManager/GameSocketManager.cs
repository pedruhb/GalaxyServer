using Galaxy.Communication.ConnectionManager.Socket_Exceptions;
using log4net;
using System;
...
namespace Galaxy.Communication.ConnectionManager
{
	...
	{
		private static readonly ILog log = LogManager.GetLogger("Galaxy.Communication.ConnectionManager");
		...
		public void Init(int portID, int maxConnections, int connectionsPerIP, IDataParser parser, bool disableNaglesAlgorithm)
		{
			...
			log.Info("» Configuração bem sucedida da porta " + portID + "! ");
			log.Info("» Conexões máximas por IP definida como " + connectionsPerIP + "! ");
		}
			...
			if (connectionListener != null)
			{
				if (_acceptConnections)
				{
					try
					{
						...
						{
							log.Info("Conexão negada de [" + replyFromComputer.RemoteEndPoint.ToString().Split(':')[0] + "] por ter várias conexões (" + ConnectionCount + ").");
						}
					}
					catch
					{
