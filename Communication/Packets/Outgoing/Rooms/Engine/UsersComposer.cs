    ...
    class UsersComposer : ServerPacket
    {
        ...
        private void WriteUser(RoomUser User)
        {
            if (!User.IsPet && !User.IsBot)
            {
                ...
                if (Habbo != null)
                {
                    if (Habbo.GetStats() != null)
                    {
                        ...
                        {
                            if (!GalaxyServer.GetGame().GetGroupManager().TryGetGroup(Habbo.GetStats().FavouriteGroupId, out Group))
                                Group = null;
                        }
                    }
                }
            ...
            else if (User.IsBot)
            {
                ...
                base.WriteString(GalaxyServer.GetUsernameById(User.BotData.ownerID)); // Owner name
        ...
        public string PetFigureForType(int Type)
        {
            ...
            switch (Type)
            {
                ...
                case 33:
                    {
                        int RandomNumber = _random.Next(1, 2);
                        switch (RandomNumber)
                        {
                            default:
                            case 1:
