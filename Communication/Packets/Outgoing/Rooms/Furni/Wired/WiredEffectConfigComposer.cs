...
namespace Galaxy.Communication.Packets.Outgoing.Rooms.Furni.Wired
{
    class WiredEffectConfigComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.WiredEffectConfigMessageComposer)
        {
            ...
            if (Box.Type == WiredBoxType.EffectMoveUser || Box.Type == WiredBoxType.EffectProgressUserAchievement)
                base.WriteInteger(0);
            else if(Box.Type == WiredBoxType.EffectMatchPosition && GalaxyServer.Tipo == 1)
				base.WriteInteger(6);
			else base.WriteInteger(20);
            ...
            else if (Box.Type == WiredBoxType.EffectBotFollowsUserBox || Box.Type == WiredBoxType.EffectBotCopiaVisual)
            {
                if (String.IsNullOrEmpty(Box.StringData))
            ...
            if (Box.Type != WiredBoxType.EffectMatchPosition &&
                ...
                Box.Type != WiredBoxType.EffectMoveUser && Box.Type != WiredBoxType.EffectBotCopiaVisual)
            {
            ...
            else if (Box.Type == WiredBoxType.EffectMatchPosition || Box.Type == WiredBoxType.EffectMoveAndRotate || Box.Type == WiredBoxType.EffectAddScore || Box.Type == WiredBoxType.EffectAddRewardPoints || Box.Type == WiredBoxType.EffectMoveToDir || Box.Type == WiredBoxType.EffectMoveUser)
            {
                IWiredCycle Cycle = (IWiredCycle)Box;
                base.WriteInteger(0);
                base.WriteInteger(WiredBoxTypeUtility.GetWiredId(Box.Type));
