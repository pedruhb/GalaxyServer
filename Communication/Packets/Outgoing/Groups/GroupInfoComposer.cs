...
namespace Galaxy.Communication.Packets.Outgoing.Groups
{
	class GroupInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.GroupInfoMessageComposer)
        {
			...
			WriteString((GalaxyServer.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId) == null) ? "Nenhum quarto encontrado ..." : GalaxyServer.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId).Name);    // room name
			...
			WriteString(GalaxyServer.GetUsernameById(Group.CreatorId));
