...
namespace Galaxy.Communication.Packets.Incoming.Help
{
    class SubmitBullyReportEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            GameClient Client = GalaxyServer.GetGame().GetClientManager().GetClientByUserID(Convert.ToInt32(UserId));
            if (Client == null)
            {
            ...
            if (Session.GetHabbo().LastAdvertiseReport > GalaxyServer.GetUnixTimestamp())
            {
            ...
            if (Session.GetHabbo().Rank <= 1)
                Session.GetHabbo().LastAdvertiseReport = GalaxyServer.GetUnixTimestamp() + 300;
            else
                Session.GetHabbo().LastAdvertiseReport = GalaxyServer.GetUnixTimestamp();

            Client.GetHabbo().AdvertisingReported = true;
            Session.SendMessage(new SubmitBullyReportComposer(0));
            //GalaxyServer.GetGame().GetClientManager().ModAlert("New advertising report! " + Client.GetHabbo().Username + " has been reported for advertising by " + Session.GetHabbo().Username +".");
            GalaxyServer.GetGame().GetClientManager().DoAdvertisingReport(Session, Client);     
            return;
        }
    }
}