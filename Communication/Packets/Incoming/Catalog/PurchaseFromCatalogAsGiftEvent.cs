...
namespace Galaxy.Communication.Packets.Incoming.Catalog
{
    public class PurchaseFromCatalogAsGiftEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
			...
			{
				Session.SendNotification("Ops! Você não pode enviar presentes para você mesmo.");
				return;
			}

			if ((DateTime.Now - Session.GetHabbo().LastGiftPurchaseTime).TotalSeconds <= 5.0)
            {
                Session.SendNotification("Você está comprando presentes muito rápido! Aguarde 5 segundos!");
            ...
            using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
            {
                ...
                switch (Item.Data.InteractionType)
                {
                    ...
                    case InteractionType.BADGE_DISPLAY:
                        ...
                        {
                            Session.SendMessage(new BroadcastMessageAlertComposer("Ei, parece que você já tem esse emblema!"));
                            return;
                        }
