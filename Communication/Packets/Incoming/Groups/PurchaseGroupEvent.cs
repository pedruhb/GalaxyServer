...
namespace Galaxy.Communication.Packets.Incoming.Groups
{
    class PurchaseGroupEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket packet)
        {
            ...
            int groupCost = Convert.ToInt32(GalaxyServer.GetGame().GetSettingsManager().TryGetValue("catalog.group.purchase.cost"));
            ...
            RoomData Room = GalaxyServer.GetGame().GetRoomManager().GenerateRoomData(RoomId);
            ...
            Group Group = null;
            if (!GalaxyServer.GetGame().GetGroupManager().TryCreateGroup(Session.GetHabbo(), Name, Description, RoomId, Badge, Colour1, Colour2, out Group))
            {
                Session.SendNotification("Houve um erro ao tentar criar este grupo.\n\nTente de novo, se você receber esta mensagem mais de uma vez fale com o moderador.\r\r");
                return;
            }
