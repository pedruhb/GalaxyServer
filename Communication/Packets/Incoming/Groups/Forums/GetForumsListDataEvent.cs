...
namespace Galaxy.Communication.Packets.Incoming.Groups
{
    class GetForumsListDataEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            {
                case 2:
                    var Forums = GalaxyServer.GetGame().GetGroupForumManager().GetForumsByUserId(Session.GetHabbo().Id);
                ...
                case 0:
                    ...
                    using (var adap = GalaxyServer.GetDatabaseManager().GetQueryReactor())
                    {
                        ...
                        adap.AddParameter("now", (int)GalaxyServer.GetUnixTimestamp());
                    ...
                    {
                        GroupForum forum;
                        if (GalaxyServer.GetGame().GetGroupForumManager().TryGetForum(Convert.ToInt32(Row["id"]), out forum))
                            forums.Add(forum);
                    }
                    break;
