...
namespace Galaxy.Communication.Packets.Incoming.Groups
{
    class DeleteGroupEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            if (Group.MemberCount >= Convert.ToInt32(GalaxyServer.GetGame().GetSettingsManager().TryGetValue("group.delete.member.limit")) && !Session.GetHabbo().GetPermissions().HasRight("group_delete_limit_override"))
            {
                Session.SendNotification("Ops seu grupo já superrous a quantidade maxima  de mebros! (" + Convert.ToInt32(GalaxyServer.GetGame().GetSettingsManager().TryGetValue("group.delete.member.limit")) + ") que un grupo puede exceder antes de ser elegible para su eliminación. Solicitar asistencia de un miembro del staff.");
                return;
            }

            Room Room = GalaxyServer.GetGame().GetRoomManager().LoadRoom(Group.RoomId);
            ...
            GalaxyServer.GetGame().GetGroupManager().DeleteGroup(Group.Id);
            ...
            if (forumEnabled)
            {
                GalaxyServer.GetGame().GetGroupForumManager().RemoveGroup(Group);
                return;
            }
