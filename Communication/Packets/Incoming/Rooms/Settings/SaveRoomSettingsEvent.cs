...
namespace Galaxy.Communication.Packets.Incoming.Rooms.Settings
{
    class SaveRoomSettingsEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            string word;
            string Name = Packet.PopString().Replace("<", "").Replace(">", "");
            ...
            Description = GalaxyServer.GetGame().GetChatManager().GetFilter().IsUnnaceptableWord(Description, out word) ? "Spam" : Description;
            ...
            int AllowPets = Convert.ToInt32(GalaxyServer.BoolToEnum(Packet.PopBoolean()));
            int AllowPetsEat = Convert.ToInt32(GalaxyServer.BoolToEnum(Packet.PopBoolean()));
            int RoomBlockingEnabled = Convert.ToInt32(GalaxyServer.BoolToEnum(Packet.PopBoolean()));
            int Hidewall = Convert.ToInt32(GalaxyServer.BoolToEnum(Packet.PopBoolean()));
            ...
            else
            {
                ...
                Room.SendMessage(new RoomVisualizationSettingsComposer(Room.WallThickness, Room.FloorThickness, GalaxyServer.EnumToBool(Room.Hidewall.ToString())));
            }

            GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_SelfModDoorModeSeen", 1);
            GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_SelfModWalkthroughSeen", 1);
            GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_SelfModChatScrollSpeedSeen", 1);
            GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_SelfModChatFloodFilterSeen", 1);
            GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(Session, "ACH_SelfModChatHearRangeSeen", 1);
        }
    }
}
