...
namespace Galaxy.Communication.Encryption.Crypto.RSA
{
    public class RSAKey
    {
        ...
        private byte[] pkcs1unpad(byte[] src, int n, Pkcs1PadType type)
        {
            ...
            {
                //.WriteLine("PKCS#1 unpad: i={0}, expected src[i]==[0,1,2], got src[i]={1}", i, src[i].ToString("X"));
                return null;
            }
            ...
            while (src[i] != 0)
            {
                if (++i >= src.Length)
                {
                    //.WriteLine("PKCS#1 unpad: i={0}, src[i-1]!=0 (={1})", i, src[i - 1].ToString("X"));
                }
            }
