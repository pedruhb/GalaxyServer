...
namespace Galaxy.HabboHotel.Items.Wired.Boxes.Effects
{
    class SetFastWalkUserBox : IWiredItem
    {
        ...
        public bool Execute(params object[] Params)
        {
            ...
            if (!User.FastWalking)
            {
                Player.GetClient().SendMessage(RoomNotificationComposer.SendBubble("wffwalk", "Super velocidade desativada.", ""));
            }
            else
            {
                Player.GetClient().SendMessage(RoomNotificationComposer.SendBubble("wffwalk", "Super velocidade ativada.", ""));
            }
            return true;
        }
    }
