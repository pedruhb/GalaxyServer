...
namespace Galaxy.HabboHotel.Items.Wired.Boxes.Effects
{
    class AddScoreBox : IWiredItem, IWiredCycle
    {
        ...
        private void TeleportUser(Habbo Player)
        {
            ...
            int currentscore = 0;
            int phblindo0 = Convert.ToInt32(StringData.Split(';')[0]);
            int phblindo1 = Convert.ToInt32(StringData.Split(';')[1]);
            int mScore = phblindo0 * phblindo1;
            ...
            if ((Instance == null || User == null ? false : !User.IsBot))
            {
                ...
                User.GetClient().SendMessage(RoomNotificationComposer.SendBubble("award", "Você acaba de ganhar " + mScore + " pontos!", ""));
