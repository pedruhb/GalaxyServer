        ...
        public static void GenerateExtradata(Item Item, ServerPacket Message)
        {
            switch (Item.GetBaseItem().InteractionType)
            {
                ...
                case InteractionType.GUILD_FORUM:
                    Group Group = null;
                    if (!GalaxyServer.GetGame().GetGroupManager().TryGetGroup(Item.GroupId, out Group))
                    {
                        Message.WriteInteger(1);
                        Message.WriteInteger(0);
                        Message.WriteString(Item.ExtraData);
                    ...
                    else
                    {
                        ...
                        Message.WriteString(GalaxyServer.GetGame().GetGroupManager().GetColourCode(Group.Colour1, true));
                        Message.WriteString(GalaxyServer.GetGame().GetGroupManager().GetColourCode(Group.Colour2, false));
                    }
                    break;
                ...
                case InteractionType.GIFT:
                    {
                        ...
                        else
                        {
                            ...
                            UserCache Purchaser = GalaxyServer.GetGame().GetCacheManager().GenerateUser(Convert.ToInt32(ExtraData[2]));
                            if (Purchaser == null)
                            {
                                Message.WriteInteger(0);
                                Message.WriteInteger(0);
                ...
                case InteractionType.TELEVISION:
                    ...
                    Message.WriteString("//game//c_images//thumb_youtube.php?video=" + GalaxyServer.GetGame().GetTelevisionManager().TelevisionList.OrderBy(x => Guid.NewGuid()).FirstOrDefault().YouTubeId + "");
                   //       Message.WriteString("");
