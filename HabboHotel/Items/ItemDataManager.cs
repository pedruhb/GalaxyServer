...
namespace Galaxy.HabboHotel.Items
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger("Galaxy.HabboHotel.Items.ItemDataManager");
                ...
                if (ItemData != null)
                {
                    foreach (DataRow Row in ItemData.Rows)
                    {
                        try
                        {
                            ...
                            bool allowStack = GalaxyServer.EnumToBool(Row["can_stack"].ToString());
                            bool allowWalk = GalaxyServer.EnumToBool(Row["is_walkable"].ToString());
                            bool allowSit = GalaxyServer.EnumToBool(Row["can_sit"].ToString());
                            bool allowRecycle = GalaxyServer.EnumToBool(Row["allow_recycle"].ToString());
                            bool allowTrade = GalaxyServer.EnumToBool(Row["allow_trade"].ToString());
                            bool allowMarketplace = Convert.ToInt32(Row["allow_marketplace_sell"]) == 1;
                            bool allowGift = Convert.ToInt32(Row["allow_gift"]) == 1;
                            bool allowInventoryStack = GalaxyServer.EnumToBool(Row["allow_inventory_stack"].ToString());
                            ...
                            bool IsRare = GalaxyServer.EnumToBool(Row["is_rare"].ToString());
                            int ClothingId = Convert.ToInt32(Row["clothing_id"]);
                            bool ExtraRot = GalaxyServer.EnumToBool(Row["extra_rot"].ToString());
                        ...
                        catch (Exception e)
                        {
                            ...
                            Console.ReadKey();
                            log.Warn("Não foi possível carregar o item " + Convert.ToInt32(Row[0]) + ", verifique o banco de dados.");
                        }
                    }
                }
            }

            //log.Info("» Items -> PRONTO!");
        }
