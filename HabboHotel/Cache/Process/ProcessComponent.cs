...
namespace Galaxy.HabboHotel.Cache.Process
{
    sealed class ProcessComponent
	{
		...
		public void Run(object State)
		{
			try
			{
				...
				// BEGIN CODE
				List<UserCache> CacheList = GalaxyServer.GetGame().GetCacheManager().GetUserCache().ToList();
				if (CacheList.Count > 0)
				{
					foreach (UserCache Cache in CacheList)
					{
						try
						{
							...
							if (Cache.isExpired())
								GalaxyServer.GetGame().GetCacheManager().TryRemoveUser(Cache.Id, out Temp);
				...
				List<Habbo> CachedUsers = GalaxyServer.GetUsersCached().ToList();
					...
					foreach (Habbo Data in CachedUsers)
					{
						try
						{
							...
							if (Data.CacheExpired())
								GalaxyServer.RemoveFromCache(Data.Id, out Temp);
