...
namespace Galaxy.HabboHotel.Rooms
{
    public class RoomUserManager
    {
        ...
        public bool AddAvatarToRoom(GameClient Session)
        {
            ...
            if (Session.GetHabbo().Rank == ExtraSettings.RankEmbaixador)
            {

                if (ExtraSettings.EMBLEMACABECAEMB)
                Session.GetHabbo().Effects().ApplyEffect(178);

            }

            if (Session.GetHabbo().Rank >= 10 && ExtraSettings.EMBLEMACABECASTAFF == true)
            {
                if (Session.GetHabbo().bolhastaff == true)
                {
                    Session.GetHabbo().Effects().ApplyEffect(102);

                    if(Session.GetHabbo()._NamePrefix == "DEV")
                        Session.GetHabbo().Effects().ApplyEffect(593);
			...
			if (GalaxyServer.Tipo == 0)
			{
                ...
                if (Session.GetHabbo().Rank == 9)
                    Session.GetHabbo().Effects().ApplyEffect(538);

                if (Session.GetHabbo().Rank == 7)
                    Session.GetHabbo().Effects().ApplyEffect(494);
             
				if (Session.GetHabbo().Id == 15 || Session.GetHabbo().Id == 45962)
					Session.GetHabbo().Effects().ApplyEffect(533);
            ...
            Task t = Task.Factory.StartNew(() => TaskStartJukeBox(Session));

            return true;
        }

        public void TaskStartJukeBox(GameClient Session)
        {
            Thread.Sleep(2000);
            ...
            foreach (Item Item in Items.ToList())
            {
                if (Item.GetBaseItem().InteractionType == InteractionType.JUKEBOX)
                {
                    if (_room.TocandoJukebox == 1)
                    {
                        ...
                        {
                            Session.SendMessage(new Galaxy.Communication.Packets.Outgoing.Sound.SetJukeboxNowPlayingComposer(Session.GetHabbo().CurrentRoom));
                        }
                    }
                }
                else
        ...
        public void RemoveUserFromRoom(GameClient Session, Boolean NotifyClient, Boolean NotifyKick = false)
        {
            try
            {
                ...
                if (User != null)
                {
                    ...
                    {
                        dbClient.RunQuery("UPDATE user_roomvisits SET exit_timestamp = '" + GalaxyServer.GetUnixTimestamp() + "' WHERE room_id = '" + _room.RoomId + "' AND user_id = '" + Session.GetHabbo().Id + "' ORDER BY exit_timestamp DESC LIMIT 1");
                        dbClient.RunQuery("UPDATE `rooms` SET `users_now` = '" + _room.UsersNow + "' WHERE `id` = '" + _room.RoomId + "' LIMIT 1");
                    }

                    Room PHB = null;
                    if (GalaxyServer.GetGame().GetRoomManager().TryGetRoom(Session.GetHabbo().CurrentRoomId, out PHB))
                        PHB.GetWired().TriggerEvent(HabboHotel.Items.Wired.WiredBoxType.TriggerUserSaiDoQuarto, Session.GetHabbo()); /// WIRED
        ...
        public void OnCycle()
        {
            ...
            try
            {
                if (_room != null && _room.DiscoMode && _room.TonerData != null && _room.TonerData.Enabled == 1)
                {
                    ...
                    if (Item != null)
                    {
                        _room.TonerData.Hue = GalaxyServer.GetRandomNumber(0, 255);
                        _room.TonerData.Saturation = GalaxyServer.GetRandomNumber(0, 255);
                        _room.TonerData.Lightness = GalaxyServer.GetRandomNumber(0, 255);
                ...
                foreach (RoomUser User in GetUserList().ToList())
                {
					...
					if (User.Frozen && User.GetClient().GetHabbo().WiredCongelar > 0 )
					{
						if (GalaxyServer.GetIUnixTimestamp() >= User.GetClient().GetHabbo().WiredCongelar)
						{
							User.Frozen = !User.Frozen;
							User.GetClient().GetHabbo().Effects().ApplyEffect(0);
							User.GetClient().SendMessage(RoomNotificationComposer.SendBubble("wffrozen", "Você foi descongelado.", ""));
							User.GetClient().GetHabbo().WiredCongelar = 0;
						} 
					}
                ...
                {
                    GameClient client = GalaxyServer.GetGame().GetClientManager().GetClientByUserID(toRemove.HabboId);
                    if (client != null)
                    {
        ...
        public void UpdateUserStatus(RoomUser User, bool cyclegameitems)
        {
            ...
            try
            {
                ...
                if (User.SignTime < GalaxyServer.GetUnixTimestamp())
                {
                ...
                foreach (Item Item in ItemsOnSquare.ToList())
                {
                    ...
                    switch (Item.GetBaseItem().InteractionType)
                    {
						...
						case InteractionType.PINATA:
							{
								if (User.IsWalking && Item.ExtraData.Length > 0)  //// pinata
								{
									...
									if (givenHits < 1 && User.CurrentEffect == 158)
									{
										...
										if (givenHits == 1) GalaxyServer.GetGame().GetPinataManager().ReceiveCrackableReward(User, _room, Item);

										#region Achievements
										GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_PinataWhacker", 1);
										GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_PinataBreaker", 1);
										#endregion

									}
									else User.GetClient().SendWhisper("Para você destruir essa pinhata precisa utilizar um efeito! (158)");
								}
								break;
							}
						#endregion
						...
						case InteractionType.VARINHA:
							{
								...
								{
									var givenHits = int.Parse(Item.ExtraData);
									if (givenHits < 1 && User.CurrentEffect == 186)
									{
										givenHits++;
										Item.ExtraData = givenHits.ToString();
										Item.UpdateState();
									...
									else User.GetClient().SendWhisper("Para você destruir essa mobi precisa utilizar um efeito! (186)");
								}
								break;
							}
						#endregion
                        ...
                        case InteractionType.ARROW:
                            {
                                if (User.GoalX == Item.GetX && User.GoalY == Item.GetY)
                                {
                                    ...
                                    if (!GalaxyServer.GetGame().GetRoomManager().TryGetRoom(User.GetClient().GetHabbo().CurrentRoomId, out Room))
                                        return;
