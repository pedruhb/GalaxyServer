...
namespace Galaxy.HabboHotel.Rooms
{
	public class Gamemap
	{
		...
		public Gamemap(Room room)
		{
			...
			mStaticModel = GalaxyServer.GetGame().GetRoomManager().GetModel(room.ModelName);
			if (mStaticModel == null)
			{
				GalaxyServer.GetGame().GetRoomManager().LoadModel(room.ModelName);
				mStaticModel = GalaxyServer.GetGame().GetRoomManager().GetModel(room.ModelName);
			}
		...
		public Point getRandomWalkableSquare()
		{
			...
			int RandomNumber = GalaxyServer.GetRandomNumber(0, walkableSquares.Count);
			int i = 0;
			...
			if (mCoordinatedItems.ContainsKey(coord))
			{
				...
				foreach (Item item in itemsFromSquare)
				{
					if (item.GetBaseItem().InteractionType == InteractionType.RENTABLE_SPACE)
						...
						if (GalaxyServer.GetGame().GetRentableSpaceManager().GetRentableSpaceItem(item.Id, out _rentableSpace))
						{
							...
							if (_rentableSpace.OwnerId == Session.GetHabbo().Id && GalaxyServer.GetUnixTimestamp() < _rentableSpace.ExpireStamp)
							{
		...
		public bool IsValidStep2(RoomUser User, Vector2D From, Vector2D To, bool EndOfPath, bool Override)
		{
			...
			if (Items.Count > 0)
			{
				...
				if (HasGroupGate)
				{
					...
					{
						Group Group = null;
						if (!GalaxyServer.GetGame().GetGroupManager().TryGetGroup(I.GroupId, out Group))
							return false;
		...
		public bool ValidTile2(int X, int Y, Item ball = null)
		{
			if (X < 0 || Y < 0 || X >= Model.MapSizeX || Y >= Model.MapSizeY)
			{
				if (ball != null)
