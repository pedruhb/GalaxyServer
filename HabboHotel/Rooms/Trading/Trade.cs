    ...
    public class Trade
    {
        ...
        public void DeliverItems()
        {
            ...
            foreach (Item I in ItemsOne.ToList())
            {
                ...
                {
                    GetTradeUser(oneId).GetClient().SendNotification(GalaxyServer.GetGame().GetLanguageManager().TryGetValue("trade_failed"));
                    GetTradeUser(twoId).GetClient().SendNotification(GalaxyServer.GetGame().GetLanguageManager().TryGetValue("trade_failed"));
                    return;
                }
                User1 += I.Id + ";";
            }
            ...
            using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
            {
                ...
                dbClient.AddParameter("1items", LogPHB1);
                dbClient.AddParameter("2items", LogPHB2);
                dbClient.RunQuery();
            }
