...
namespace Galaxy.HabboHotel.Rooms.AI.Types
{
    public class PetBot : BotAI
    {
            ...
            if (User.GetClient() != null && User.GetClient().GetHabbo() != null)
            {
                ...
                if (Pet != null)
                {
                    if (User.GetClient().GetHabbo().Username == Pet.PetData.OwnerName)
                    {
                        ...
                        string[] Speech = GalaxyServer.GetGame().GetChatManager().GetPetLocale().GetValue("welcome.speech.pet" + Pet.PetData.Type);
        ...
        public override void OnTimerTick()
        {
            ...
            if (SpeechTimer <= 0)
            {
                ...
                if (Pet != null)
                {
                    ...
                    string[] Speech = GalaxyServer.GetGame().GetChatManager().GetPetLocale().GetValue("speech.pet" + Pet.PetData.Type);
                    string rSpeech = Speech[RandomNumber.GenerateRandom(0, Speech.Length - 1)];
                ...
                SpeechTimer = GalaxyServer.GetRandomNumber(20, 120);
            }
            else
            {
                SpeechTimer--;
        ...
        public override void OnUserSay(RoomUser User, string Message)
        {
            ...
            if ((Message.ToLower().StartsWith(Pet.PetData.Name.ToLower() + " ") && User.GetClient().GetHabbo().Username.ToLower() == Pet.PetData.OwnerName.ToLower()) || (Message.ToLower().StartsWith(Pet.PetData.Name.ToLower() + " ") && GalaxyServer.GetGame().GetChatManager().GetPetCommands().TryInvoke(Message.Substring(Pet.PetData.Name.ToLower().Length + 1)) == 8))
            {
                string Command = Message.Substring(Pet.PetData.Name.ToLower().Length + 1);

                int r = RandomNumber.GenerateRandom(1, 8); // Made Random
                if (Pet.PetData.Energy > 10 && r < 6 || Pet.PetData.Level > 15 || GalaxyServer.GetGame().GetChatManager().GetPetCommands().TryInvoke(Command) == 8)
                {
                    RemovePetStatus(); // Remove Status

                    switch (GalaxyServer.GetGame().GetChatManager().GetPetCommands().TryInvoke(Command))
                    {
                        ...
                        case 1:
                            ...
                            //int randomX = GalaxyServer.GetRandomNumber(0, GetRoom().Model.MapSizeX);
                            //int randomY = GalaxyServer.GetRandomNumber(0, GetRoom().Model.MapSizeY);
                            Point nextCoord = GetRoom().GetGameMap().getRandomWalkableSquare();
                        ...
                        default:
                            string[] Speech = GalaxyServer.GetGame().GetChatManager().GetPetLocale().GetValue("pet.unknowncommand");
                ...
                else
                {
                    ...
                    {
                        string[] Speech = GalaxyServer.GetGame().GetChatManager().GetPetLocale().GetValue("pet.tired");
                    ...
                    {
                        string[] Speech = GalaxyServer.GetGame().GetChatManager().GetPetLocale().GetValue("pet.lazy");
