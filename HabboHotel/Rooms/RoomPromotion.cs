...
namespace Galaxy.HabboHotel.Rooms
{
    public class RoomPromotion
    {
        ...
        public RoomPromotion(string Name, string Desc, int CategoryId)
        {
            ...
            this._timestampStarted = GalaxyServer.GetUnixTimestamp();
            this._timestampExpires = (GalaxyServer.GetUnixTimestamp()) + (Convert.ToInt32(GalaxyServer.GetGame().GetSettingsManager().TryGetValue("room.promotion.lifespan")) * 60);
        ...
        {
            get { return (this.TimestampExpires - GalaxyServer.GetUnixTimestamp()) < 0; }
        }

        public int MinutesLeft
        {
            get { return Convert.ToInt32(Math.Ceiling((this.TimestampExpires - GalaxyServer.GetUnixTimestamp()) / 60)); }
        }
