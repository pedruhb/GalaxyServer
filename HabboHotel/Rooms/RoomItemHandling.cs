...
namespace Galaxy.HabboHotel.Rooms
{
    public class RoomItemHandling
    {
        ...
        public void LoadFurniture()
        {
            ...
            foreach (Item Item in Items.ToList())
            {
                ...
                if (Item.IsFloorItem)
                {
                    if (!_room.GetGameMap().ValidTile(Item.GetX, Item.GetY))
                    {
                        ...
                        GameClient Client = GalaxyServer.GetGame().GetClientManager().GetClientByUserID(Item.UserID);
                        if (Client != null)
                        {
            ...
            foreach (Item Item in _floorItems.Values.ToList())
            {
                ...
                else if (Item.IsWired)
                {
					...
					if (Item.GetBaseItem().WiredType.ToString() == "EffectMatchPosition")
						WiredMPCount++;
		...
		public bool SetFloorItem(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false)
		{
			...
			if(Session.GetHabbo().StackHeight > 0)
			...
			if (newItem)
			{
				...
				{
					if (Session != null)
						Session.SendNotification(GalaxyServer.GetGame().GetLanguageManager().TryGetValue("room.item.already_placed"));
					_room.GetGameMap().RemoveFromMap(Item);
					return true;
				}
		...
		public bool SetFloorItemWired(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false)
		{
			bool NeedsReAdd = false;
			...
			Dictionary<int, ThreeDCoord> AffectedTiles = Gamemap.GetAffectedTiles(Item.GetBaseItem().Length, Item.GetBaseItem().Width, Item.GetX, Item.GetY, newRot);
            ...
            if (newItem)
            {
                if (_floorItems.ContainsKey(Item.Id))
                {
                    if (Session != null && ball == false)
                        Session.SendNotification(GalaxyServer.GetGame().GetLanguageManager().TryGetValue("room_item_placed"));
                    _room.GetGameMap().RemoveFromMap(Item);
                    return true;
                }
        ...
        public List<Item> RemoveItemsCommand(GameClient Session, int IdMobi)
        {
            List<Item> Items = new List<Item>();
        ...
        public List<Item> RemoveAllWireds(GameClient Session)
        {
            List<Item> Items = new List<Item>();
