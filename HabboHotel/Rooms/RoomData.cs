...
namespace Galaxy.HabboHotel.Rooms
{
    public class RoomData
    {
        ...
        public void Fill(DataRow Row)
        {
			...
			Group G = null;
            if (GalaxyServer.GetGame().GetGroupManager().TryGetGroup(Convert.ToInt32(Row["group_id"]), out G))
                Group = G;
            else
                Group = null;
            ...
            mModel = GalaxyServer.GetGame().GetRoomManager().GetModel(ModelName);

            this.SPushEnabled = GalaxyServer.EnumToBool(Row["push_enabled"].ToString());
            this.PushEnabled = GalaxyServer.EnumToBool(Row["push_enabled"].ToString());
            this.SPullEnabled = GalaxyServer.EnumToBool(Row["spull_enabled"].ToString());



                using (IQueryAdapter phbpull = GalaxyServer.GetDatabaseManager().GetQueryReactor())
                {
                    phbpull.SetQuery("SELECT pull_enabled FROM rooms WHERE id = @id");
                    phbpull.AddParameter("id", Id);
                    this.PullEnabled = GalaxyServer.EnumToBool(phbpull.getString().ToString());
                }

            


            this.EnablesEnabled = GalaxyServer.EnumToBool(Row["enables_enabled"].ToString());
            this.RespectNotificationsEnabled = GalaxyServer.EnumToBool(Row["respect_notifications_enabled"].ToString());
            this.PetMorphsAllowed = GalaxyServer.EnumToBool(Row["pet_morphs_allowed"].ToString());
			this.HideWired = GalaxyServer.EnumToBool(Row["hide_wired"].ToString());
			this.AtravessarCadeira = GalaxyServer.EnumToBool(Row["atravessar_cadeira"].ToString());
