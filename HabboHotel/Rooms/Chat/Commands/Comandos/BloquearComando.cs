...
{
	class BloquearComando : IChatCommand
	{
		...
		{
			get { return "Bloqueia o comando no quarto."; }
		}
		...
		public void Execute(GameClients.GameClient Session, Rooms.Room Room, string[] Params)
		{
			...
			using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
			{
				...
				if (BlockCMD != null)
				{
					Session.SendWhisper("Esse comando já está bloqueado.");
					return;
				} 
					else
				{
					dbClient.SetQuery("INSERT INTO room_blockcmd (`room`, `command`) VALUES (@room, @command);");
					dbClient.AddParameter("command", comando);
					dbClient.AddParameter("room", Session.GetHabbo().CurrentRoomId);
					dbClient.RunQuery();

					Session.SendWhisper("O comando foi bloqueado com sucesso.");
					return;
				}
			}
