...
using Galaxy.Communication.Packets.Outgoing.Rooms.Freeze;
...
namespace Galaxy.HabboHotel.Rooms.Games.Freeze
{
    public class Freeze
    {
        private Room _room;
        ...
        {
            return ExitTeleports.Values.ToList()[GalaxyServer.GetRandomNumber(0, ExitTeleports.Count - 1)];
        }
        ...
        private void CountTeamPoints()
        {
            ...
            foreach (RoomUser User in this._room.GetRoomUserManager().GetUserList().ToList())
            {
                ...
                GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FreezePlayer", 1);
            }
        }
                ...
                case FreezePowerUp.HEART:
                    {
                        if (User.FreezeLives < 5)
                        {
                            ...
                            GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_FreezeWinner", 1);
                        }
        ...
        private void FreezeUser(RoomUser User)
        {
            ...
            _room.GetGameManager().AddPointToTeam(User.Team, -10);
            User.ApplyEffect(12);
            GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_EsA", 1);
