...
using Galaxy.Utilities.Enclosure;
using System.Linq;
...
namespace Galaxy.HabboHotel.Rooms.Games.Banzai
{
    public class BattleBanzai
    {
        ...
        public void BanzaiStart()
        {
            ...
            timestarted = GalaxyServer.GetUnixTimestamp();
        ...
        public void BanzaiEnd(bool userTriggered = false)
        {
            ...
            if (winners != TEAM.NONE)
            {
                ...
                foreach (RoomUser User in Winners.ToList())
                {
                    ...
                    {
                        if (GalaxyServer.GetUnixTimestamp() - timestarted > 5)
                        {
                            GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallTilesLocked", User.LockedTilesCount);
                            GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallPlayer", 1);
                        }
                    }
                    if (winners == TEAM.BLUE)
                    {
                        ...
                        {
                            if (GalaxyServer.GetUnixTimestamp() - timestarted > 5)
                                GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_BattleBallWinner", 1);
