...
namespace Galaxy.HabboHotel.Rooms.TraxMachine
{
    public class RoomTraxManager
    {
        ...
        public int TimestampSinceStarted
        {
            get
            {
                return (int)GalaxyServer.GetUnixTimestamp() - StartedPlayTimestamp;
            }
        }
        ...
        public void PlayPlaylist()
        {
            if (Playlist.Count == 0)
                return;
            StartedPlayTimestamp = (int)GalaxyServer.GetUnixTimestamp();
            SelectedDiscItem = null;
            IsPlaying = true;
            Room.TocandoJukebox = 1;
            SetJukeboxesState();
        }
