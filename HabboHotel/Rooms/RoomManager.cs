...
namespace Galaxy.HabboHotel.Rooms
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger("Galaxy.HabboHotel.Rooms.RoomManager");
   ...
   //         log.Info("» Quartos -> PRONTO!");
        ...
        public void OnCycle()
        {
            try
            {
                ...
                GalaxyServer.GetGame().RoomManagerCycleEnded = true;
            }
            catch (Exception e)
            {
            ...
            using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
            {
                ...
                foreach (DataRow Row in Data.Rows)
                {
                    ...
                    _roomModels.Add(Modelname, new RoomModel(Convert.ToInt32(Row["door_x"]), Convert.ToInt32(Row["door_y"]), (Double)Row["door_z"], Convert.ToInt32(Row["door_dir"]),
                        Convert.ToString(Row["heightmap"]), Convert.ToString(Row["public_items"]), GalaxyServer.EnumToBool(Row["club_only"].ToString()), Convert.ToString(Row["poolmap"]), Convert.ToInt32(Row["wall_height"])));
                }
            }
        }
            ...
            string wallpaper = "0.0", string floor = "0.0", string landscape = "0.0", int wallthick = 0, int floorthick = 0)
        {
            if (!_roomModels.ContainsKey(Model))
            {
                Session.SendNotification(GalaxyServer.GetGame().GetLanguageManager().TryGetValue("room.creation.model.not_found"));
                return null;
            }

            if (Name.Length < 3)
            {
                Session.SendNotification(GalaxyServer.GetGame().GetLanguageManager().TryGetValue("room.creation.name.too_short"));
                return null;
            }
        ...
        public void Dispose()
        {
            ...
            foreach (Room Room in this._rooms.Values.ToList())
            {
                ...
                Console.Clear();
                log.Info("<<- "+GalaxyServer.HotelName+" ->> SALVANDO ITENS: " + String.Format("{0:0.##}", ((double)i / length) * 100) + "%");
                i++;
            }
