...
using Galaxy.HabboHotel.Catalog.Clothing;
...
namespace Galaxy.HabboHotel.Catalog
{
    public class CatalogManager
    {
        private static readonly ILog log = LogManager.GetLogger("Galaxy.HabboHotel.Catalog.CatalogManager");
        ...
        public void Init(ItemDataManager ItemDataManager)
        {
            ...
            using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (CatalogueItems != null)
                {
                    foreach (DataRow Row in CatalogueItems.Rows)
                    {
                        ...
                        if (BaseId == 0 && PredesignedId > 0)
                        {
                            ...
                            if (roomPack.CatalogItems.Contains(";"))
                            {
                                ...
                                foreach (var item in itemArray)
                                {
                                    ...
                                    {
                                        log.Error("Ops, o item id {" + ItemId + "} está com um erro! ele será apagado.");
                                       dbClient.runFastQuery("DELETE FROM catalog_items WHERE id = '" + ItemId + "' LIMIT 1");
                                        continue;
                                    }
                        ...
                        this._items[PageId].Add(Convert.ToInt32(Row["id"]), new CatalogItem(Convert.ToInt32(Row["id"]), Convert.ToInt32(Row["item_id"]),
                            ...
                            Convert.ToInt32(Row["cost_gotw"]), Convert.ToInt32(Row["amount"]), Convert.ToInt32(Row["limited_sells"]), Convert.ToInt32(Row["limited_stack"]), GalaxyServer.EnumToBool(Row["offer_active"].ToString()),
                ...
                dbClient.SetQuery("SELECT `id`,`parent_id`,`caption`,`page_link`,`visible`,`enabled`,`min_rank`,`min_vip`,`icon_image`,`page_layout`,`page_strings_1`,`page_strings_2` FROM `catalog_pages` ORDER BY `order_num`, `caption`");
