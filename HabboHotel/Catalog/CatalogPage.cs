...
namespace Galaxy.HabboHotel.Catalog
{
    public class CatalogPage
    {
              ...
              ref Dictionary<int, int> flatOffers)
        {
            ...
            _caption = Caption.Replace("HOTELNAME", GalaxyServer.HotelName);
            _pageLink = PageLink;
            _icon = Icon;
            _minRank = MinRank;
            _minVIP = MinVIP;
            ...
            _template = Template;
            string String1 = PageStrings1.Replace("HOTELNAME", GalaxyServer.HotelName);
            string String2 = PageStrings2.Replace("HOTELNAME", GalaxyServer.HotelName);


           

            foreach (string Str in String1.Split('|'))
            {
                if (_pageStrings1 == null) { _pageStrings1 = new List<string>(); }
                _pageStrings1.Add(Str);
            }

            foreach (string Str in String2.Split('|'))
            {
