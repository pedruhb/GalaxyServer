...
namespace Galaxy.HabboHotel.Users.Process
{
    sealed class ProcessComponent
	{
		private static readonly ILog log = LogManager.GetLogger("Galaxy.HabboHotel.Users.Process.ProcessComponent");
		...
		public void Run(object State)
		{
			try
			{
                ...
                if (_player.GetStats().RespectsTimestamp != DateTime.Today.ToString("MM/dd"))
                {
					...
					if (_player.Username.ToLower() == ExtraSettings.ReiniciarPermissao)
						DailyRespects = 9999999;

					_player.GetStats().DailyRespectPoints = DailyRespects;
                    _player.GetStats().DailyPetRespectPoints = DailyRespects;

                    using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
                    {
                        dbClient.RunQuery("UPDATE `user_stats` SET `dailyRespectPoints` = '" + DailyRespects + "', `dailyPetRespectPoints` = '" + DailyRespects + "', `respectsTimestamp` = '" + DateTime.Today.ToString("MM/dd") + "' WHERE `id` = '" + _player.Id + "' LIMIT 1");
                    }
				...
				if (_player.GetClient() != null)
					GalaxyServer.GetGame().GetAchievementManager().ProgressAchievement(_player.GetClient(), "ACH_AllTimeHotelPresence", 1);
