...
using Galaxy.HabboHotel.Users.Authenticator;
    ...
    public class UserDataFactory
    {
        public static UserData GetUserData(string SessionTicket, out byte errorCode)
        {
            ...
            using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GalaxyServer.GetGame().GetClientManager().GetClientByUserID(UserId) != null)
                {
                    errorCode = 2;
                    GalaxyServer.GetGame().GetClientManager().GetClientByUserID(UserId).Disconnect();
                    return null;
                }
            ...
            foreach (DataRow dRow in dFriends.Rows)
            {
                ...
                bool friendHideOnline = GalaxyServer.EnumToBool(dRow["hide_online"].ToString());
                bool friendHideRoom = GalaxyServer.EnumToBool(dRow["hide_inroom"].ToString());
            ...
            {
                rooms.Add(GalaxyServer.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(dRow["id"]), dRow));
            }
            ...
            errorCode = 0;
            return new UserData(UserId, Achievements, favouritedRooms, badges, friends, requests, rooms, quests, user, Relationships, subscriptions, tags);
        }
        ...
        public static UserData GetUserData(int UserId)
        {
            ...
            using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
            {
                ...
                GalaxyServer.GetGame().GetClientManager().LogClonesOut(Convert.ToInt32(UserId));
            ...
            Habbo user = HabboFactory.GenerateHabbo(dUserInfo, UserInfo);
            return new UserData(UserId, Achievements, FavouritedRooms, Badges, Friends, FriendRequests, Rooms, Quests, user, Relationships, subscriptions, tags);
        }
    }
}