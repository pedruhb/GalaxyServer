...
namespace Galaxy.HabboHotel.Moderation
{
    ...
    {
        private static ILog log = LogManager.GetLogger("Galaxy.HabboHotel.Moderation.ModerationManager");
            ...
            using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetBans != null)
                {
                    foreach (DataRow dRow in GetBans.Rows)
                    {
                        ...
                        if (Ban != null)
                        {
                            if (expires > GalaxyServer.GetUnixTimestamp())
                            {
     ...
     //       log.Info("» PRONTO! " + (this._userPresets.Count + this._roomPresets.Count) + " presets de moderação.");
     //       log.Info("» PRONTO! " + this._userActionPresetCategories.Count + " categorias de moderação.");
    //        log.Info("» PRONTO! " + this._userActionPresetMessages.Count + " Msj. de pré-ajuste de ação de moderação.");
            log.Info("» Em cache: " + this._bans.Count + " usuários e máquinas banidas!");
        }
        ...
        public void ReCacheBans()
        {
            ...
            log.Info("Em cache " + this._bans.Count + " banimentos de usuário ou máquinas.");
        }
        ...
        public void BanUser(string Mod, ModerationBanType Type, string BanValue, string Reason, double ExpireTimestamp)
        {
            ...
            using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("REPLACE INTO `bans` (`bantype`, `value`, `reason`, `expire`, `added_by`,`added_date`) VALUES ('" + BanType + "', '" + BanValue + "', @reason, " + ExpireTimestamp + ", '" + Mod + "', '" + GalaxyServer.GetUnixTimestamp() + "');");
        ...
        public bool MachineBanCheck(string MachineId)
        {
            ...
            if (GalaxyServer.GetGame().GetModerationManager().IsBanned(MachineId, out MachineBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        GalaxyServer.GetGame().GetModerationManager().RemoveBan(MachineId);
                        return false;
                    }
                }
            }
        ...
        public bool UsernameBanCheck(string Username)
        {
            ...
            if (GalaxyServer.GetGame().GetModerationManager().IsBanned(Username, out UsernameBanRecord))
            {
                DataRow BanRow = null;
                using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (BanRow == null)
                    {
                        GalaxyServer.GetGame().GetModerationManager().RemoveBan(Username);
                        return false;
                    }
                }
            }
