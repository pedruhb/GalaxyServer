...
namespace Galaxy.HabboHotel.Subscriptions
{
    internal class ClubManager
    {
        ...
        internal void AddOrExtendSubscription(string SubscriptionId, int DurationSeconds, GameClient Session)
        {
            ...
            var clientByUserId = GalaxyServer.GetGame().GetClientManager().GetClientByUserID(UserId);
            if (Subscriptions.ContainsKey(SubscriptionId))
            {
                Subscription subscription = Subscriptions[SubscriptionId];
                ...
                {
                    subscription.SetEndTime((int)GalaxyServer.GetUnixTimestamp() + DurationSeconds);
                }
                ...
                GalaxyServer.GetGame().GetAchievementManager().TryProgressHabboClubAchievements(clientByUserId);
            }
            else
            {
                int unixTimestamp = (int)GalaxyServer.GetUnixTimestamp();
                int timeExpire = (int)GalaxyServer.GetUnixTimestamp() + DurationSeconds;
