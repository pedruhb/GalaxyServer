...
namespace Galaxy.HabboHotel.Groups.Forums
{
    public class GroupForumManager
    {
        ...
        public bool TryGetForum(int Id, out GroupForum Forum)
        {
            ...
            Group Gp;
            if (!GalaxyServer.GetGame().GetGroupManager().TryGetGroup(Id, out Gp))
                return false;
        ...
        {
            using (IQueryAdapter queryReactor = GalaxyServer.GetDatabaseManager().GetQueryReactor())
            {
        ...
        {
            GroupForum F;
            return GalaxyServer.GetGame().GetGroupManager().GetGroupsForUser(Userid).Where(c => TryGetForum(c.Id, out F)).Select(c => GetForum(c.Id)).ToList();
        }
    }
}
