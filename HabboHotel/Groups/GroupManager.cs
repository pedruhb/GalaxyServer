...
namespace Galaxy.HabboHotel.Groups
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger("Galaxy.HabboHotel.Groups.GroupManager");
            ...
            lock (this._groupLoadingSync)
            {
                ...
                using (IQueryAdapter dbClient = GalaxyServer.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    if (Row != null)
                    {
                        ...
                        catch { created = GalaxyServer.GetIUnixTimestamp(); }
                        Group = new Group(
        ...
        {
            Group = new Group(0, Name, Description, Badge, RoomId, Player.Id, (int)GalaxyServer.GetUnixTimestamp(), 0, Colour1, Colour2, 0, false, false);
