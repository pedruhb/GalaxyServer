...
namespace Galaxy.Core
{
    class CatalogSettings
    {
        ...
        private static readonly ILog log = LogManager.GetLogger("Galaxy.Core");

        public static bool RunCatalogSettings()
        {
            foreach (var @params in from line in File.ReadAllLines("Config/catalogo.ini", Encoding.Default) where !String.IsNullOrWhiteSpace(line) && line.Contains("=") select line.Split('='))
            {
                switch (@params[0])
                {
